...
int cfg_parse_global(const char *file, int linenum, char **args, int kwm)
{
	...
	/* end of user/group name handling*/
	...
	else if (!strcmp(args[0], "redis_server")) {  /* redis server address */
		if (*(args[1]) == 0 ) {
			Alert("parsing [%s:%d] : '%s' expects <address> and [<port>] as arguments.\n", file, linenum, args[0]);
			err_code |= ERR_ALERT | ERR_FATAL;
			goto out;
		}
		...
		global.redis_srv_addr = strdup(args[1]);

		if (*(args[2]) == 0) {
			global.redis_srv_port=6329;
		} else {
			global.redis_srv_port = atol(args[2]);
			if (global.redis_srv_port < 1024 || global.redis_srv_port > 65535) {
				Alert("parsing [%s:%d]: 'spread-checks' needs a positive value in range 1024..65535.\n", file, linenum);
				err_code |= ERR_ALERT | ERR_FATAL;
			}
		}
	}
